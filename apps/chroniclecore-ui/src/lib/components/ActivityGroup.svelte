<script lang="ts">
    ...
    $: profileColor =
        group.color || getProfileColor(group.profile_name || "Unassigned");
    $: uniqueApps = [
        ...new Set(
            group.apps || group.activities.map((a) => a.primary_app_name),
        ),
    ];
</script>
...
>
    <!-- Group Header -->
    <button
        class="w-full px-4 py-3 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-left"
    ...
    >
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
            {#each uniqueApps.slice(0, 4) as app}
                <div
                    class="w-6 h-6 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center"
                    title={app}
                >
        ...
        <div class="flex-shrink-0">
            <svg
                class="w-5 h-5 text-slate-400 transition-transform {expanded
                    ? 'rotate-180'
                    : ''}"
                fill="none"
                stroke="currentColor"
    ...
    {#if expanded}
        ...
        >
            {#each group.activities as activity}
                <div
                    class="px-4 py-2 pl-20 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-700"
                >
                    <!-- Time -->
                    <div class="w-24 text-xs text-slate-500 font-mono">
                        {formatTime(activity.ts_start)} - {formatTime(
                            activity.ts_end,
                        )}
                    </div>
                    ...
                    <!-- App Icon -->
                    <div
                        class="w-6 h-6 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        >
                            {activity.description ||
                                activity.title_summary ||
                                activity.primary_app_name}
                        </p>
                    </div>
                    ...
                    <!-- Confidence -->
                    <span
                        class="text-xs px-2 py-0.5 rounded {getConfidenceBadge(
                            activity.confidence,
                        ).bg} {getConfidenceBadge(activity.confidence).text}"
                    >
                        {activity.confidence}
                    </span>
                </div>
